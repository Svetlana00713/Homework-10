#Задание на выбор или дополнительное про классы (тема 10ого 
# семинара)|
# Напишите класс Dragon (Дракон), экземпляр которого при 
# инициализации принимaет аргументы: рост, огнеопасность, цвет.
# Класс обеспечивает выполнение методов (dr — экземпляр класса)
# экземпляры можно сравнивать: сначала по росту, затем по огне-
# опасности, затем по цвету, по алфавиту. Экземпляры класса 
# можно складывать: dr2 =dr + dr1. при этом возвращается новый 
# экземпляр со значениями атрибутов: цвет меньший по алфавиту;
# рост - среднее арифметическое из двух округлённое до целого 
# вниз, огнеопасность - большее из двух; из экземпляра класса 
# можно вычесть число: dr -= number, из роста вычитается целая 
# честь от деления роста на число, к огнеопасности прибавляется
# остаток от деления огнеопасности на число. Экземпляр можно 
# вызвать с аргументом-строкой - возвращается строка-аргумент, 
# повторенная количество раз, равное значению атрибута огнеопа-
# сность; change_color() - вызывается c аргументом - цветом, 
# на который нужно поменять имеющийся цвет; str- возвращает 
# строку: Dragon with height «рост», danger <огнеопасность> 
# and color «цвет». repr- возвращaет строку: Dragon(‹рост>, 
# <огнеопасность>, <цвет>).
# Пример
